...
import { axios_, extractData, updateFormData, validateForm, ResetFormData, updateFormOnSelection } from "../../utilities/utll";
...
export default function EmployeeMaster() {
	...
	function handleOnChange() {
		...
		if (dropdown[0]?.name === "Country" && dropdown[0]?.selectedOptions) {
			setFormData(prev => {
				let obj = { ...prev };
				obj.forms[0][0].rows[0].controls[10].fetch.api = "Master/State/GetByIdData";
			...
			setFormData(prev => {
				let obj = { ...prev };
				obj.forms[0][0].rows[0].controls[11].fetch.api = "Master/City/GetByIdData";
