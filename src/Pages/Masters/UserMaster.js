...
import { axios_, extractData, updateFormData, updateFormOnSelection, validateForm, ResetFormData } from "../../utilities/utll";
...
export default function UserMaster() {
		...
		if (dropdown[0]?.name === "userType") {
			if (dropdown[0].selectedOptions[0] === 1) {
				setFormData(prev => {
					...
					obj.forms[0][0].rows[0].controls[2].fetch.api = "Master/Employee/GetByIdData";
					obj.forms[0][0].rows[0].controls[2].fetch.data = { CompanyID };
					obj.forms[0][0].rows[0].controls[2].fetch.fields = ["EmployeeCode", "EmployeeName"];
					return obj;
				});
				...
				setFormData(prev => {
					...
					obj.forms[0][0].rows[0].controls[2].fetch.data = { CompanyID };
					obj.forms[0][0].rows[0].controls[2].fetch.fields = ["ID", "RemoteCmpName"];
					return obj;
				});
			}
		}
	...
	async function handleSubmit() {
		...
		if (await validateForm(setFormData)) {
			...
			if (inputData.User.Password != inputData.User.confirmPassword) {
				...
				updateFormOnSelection(setFormData, 0, "confirmPassword", "isValid", false);
				return;
			}
			try {
	...
	function handleEdit(record) {
		...
		Object.entries(record[1]).forEach(entry => {
			updateFormOnSelection(setFormData, 0, entry[0], "value", record[1][entry[0]] ? record[1][entry[0]] : "");
		});
