...
import { updateFormOnSelection, updateFormData, dateFormat, extractData } from "../../utilities/utll";
export default function Transaction() {
	...
	function handleOnChange() {
		...
		if (dropdown[0]?.name === "Period" && dropdown[0]?.selectedOptions) {
			updateFormOnSelection(setFormData, formItemIndex, "FromDate", "value", dateFormat(dropdown[0]?.selectedOptions[2]));
			updateFormOnSelection(setFormData, formItemIndex, "ToDate", "value", dateFormat(dropdown[0]?.selectedOptions[3]));
			// Enable/Disable Date Controls
			if (dropdown[0]?.selectedOptions[1] === "Custom") {
				updateFormOnSelection(setFormData, formItemIndex, "submit", "visible", true);
				updateFormOnSelection(setFormData, formItemIndex, "FromDate", "enabled", true);
				updateFormOnSelection(setFormData, formItemIndex, "ToDate", "enabled", true);
			} else {
				updateFormOnSelection(setFormData, formItemIndex, "submit", "visible", false);
				updateFormOnSelection(setFormData, formItemIndex, "FromDate", "enabled", false);
				updateFormOnSelection(setFormData, formItemIndex, "ToDate", "enabled", false);
			}
		}
		tbldata && tbldata["transactionsTable"] && delete tbldata["transactionsTable"];
