		...
		<>
			<div className={styles.tabContainer}>
				<div className={styles.tabs}>
					<div className={styles.tabButtonRow} ref={buttonRowRef} id='scrollRow'>
						{React.Children.map(children, (child, index) => {
							return (
								<React.Fragment key={`tabButton${index}`}>
									<div
										key={`tabButton_${index}`}
										className={`${styles.tabButton} ${activeTab.name === child.props.title ? styles.active : styles.inactive}`}
										onClick={e => {
											e.preventDefault();
					...
					{React.Children.map(children, (child, index) => {
						return (
							<div id={`tabBox${index}`} key={`tabBox${index}`} className={`${styles.tabBox} ${activeTab.name === child.props.title ? styles.activeTab : styles.InactiveTab}`}>
