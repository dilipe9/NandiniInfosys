...
function Select({ fetch = {}, selectorText = "", options = [], isError = false, title = null, name = "", frmIndex = 0, value = -1, onChange = () => {} }, inputRef) {
	...
	function handleOnChange(e) {
		...
		let selectedOptions = selectionOption.filter(option => option[0] == e.target.value)[0];
	...
	return (
        ...
        /> */}
				<select value={selected} onChange={handleOnChange} key={`select_${name}`} name={name} ref={inputRef} className={`${styles.select} ${isError ? styles.error : ""}`}>
					<option key={`default_${name}`} value={-1} styles={{ "text-align": "center" }}>
						--{selectorText}--
					</option>
					...
					{loading ? (
						...
						selectionOption.map((option, index) => (
							<option key={`${index}_${name}`} value={option[0]} disabled={option?.enabled ? option?.enabled : false}>
								{option[1]}
							</option>
						))
					)}
