...
export const axios_ = axios.create({
	baseURL: "https://nirmalapi.nify.io/api/",
});
			...
			subForms.forEach((form, formIndex) => {
				if (form.name) {
					if (form.type === "parent") {
						...
						if (!extractedData[parentName][form.name]) extractedData[parentName][form.name] = [];
						...
						row.controls.forEach(control => {
							...
							if (type !== "button" && type !== "table") {
								if (form.name === parentName) {
									...
									extractedData[parentName][form.name][extractedData[parentName][form.name].length - 1][name] = value;
								}
							}
						});
					});
...
export const updateFormOnSelection = (stateUpdateFunction, formItemIndex, controlName, propertyName, propertyValue) => {
	stateUpdateFunction(prev => {
		let obj = { ...prev };
		obj.forms.forEach(form => {
			form.forEach((forItem, formIndex) => {
